...
export function LinkContainer({ 
    ...
    return (
        <div class="p-9 bg-yellow-100 dark:bg-yellow-900 rounded-2xl relative shadow-lg dark:shadow-slate-900" key={link.id}>
            ...
            <p class="group font-bold flex flex-wrap">
                <a href={link.url} target="_blank" class="text-gray-700 dark:text-gray-100 flex gap-2 flex-row items-center group-hover:text-yellow-700 dark:group-hover:text-yellow-200">
                    {link.title ?? extractDomainName(link.url ?? '')}
                </a>
                <svg class="w-6 h-6 flex-none opacity-0 group-hover:opacity-100 dark:group-hover:stroke-yellow-200" viewBox="0 0 24 24" fill="none"><path d="M9.75 15.25L15.25 9.75M15.25 9.75H10.85M15.25 9.75V14.15" stroke="#7e22d8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" class="dark:group-hover:stroke-yellow-200"></path></svg>
            </p>
